...
const debug = require('debug')('athletesApp: storage');
...
storage.fetchItem = function(schemaName, id){
  ...
  let path = `./data/${schemaName}`;
  return fs.readFileProm(`${path}/${id}.json`)
...
storage.deleteItem = function(schemaName, id){
  ...
  let path = `./data/${schemaName}`;
  return fs.unlinkProm(`${path}/${id}.json`)
  .catch(err => Promise.reject(createError(404, err.message)));
};
